...
fn bus_tool_descriptors() -> Value {
    let mut tools = json!([
        ...
        {
            "name": "review_new",
            "description": "Compose a Review Board — a concise, keyboard-navigable review DOCUMENT the user reads in the app, for EXPLAINING CODE to them: what a change does, how the pieces fit, what to read first. Returns `{ id, slug, dir, path }`; `path` is a `review.md` that ALREADY EXISTS (seeded with a frontmatter stamp) inside a durable bundle directory — EDIT it with your normal edit tool, then call review_register { id }. The document is ordinary Markdown plus typed fenced blocks: ```review:walkthrough (title + a `steps:` list of `- at: path:line` / `say: why it matters` — the reading order), ```review:finding (severity blocking|non-blocking|suggestion, title, optional file/line; prose after the fence explains it), ```review:decision (question + an `options:` list, optional 1-based `recommended:`), ```review:metrics (flat `label: value` rows), ```review:chart (kind bar|line|sparkline, optional title, `data:` map of label → number), ```review:svg (a static diagram), and a plain ```diff fence. Make the Board a SKIM, not an essay: start with `## สรุป` and 3–5 short bullets covering what changed, the verdict, and the next action. Add one walkthrough with only the 3–5 most important steps when the subject spans multiple files. Each actionable finding gets one block whose title states the concrete impact and whose detail uses at most three short sentences: evidence, impact, suggested fix. Do not repeat the same point across the summary, walkthrough, and findings. Omit process narration, empty categories, zero-count commentary, metrics, charts, and diagrams unless a visual relationship genuinely beats concise prose. Exceed these limits only when the user explicitly asks for an exhaustive review. Findings and decisions are OPTIONAL additions — zero findings is a perfectly good review. Use it when the user asks you to explain, walk through, or review something, or to synthesize a #review. Do NOT compose one unsolicited, at every turn end, or after every edit — unwanted Boards are reviewer fatigue and clutter on disk. Write assets into `<dir>/assets/` if you need images. LANGUAGE: write everything the human reads — prose, `say:` text, finding titles and the prose under them, decision questions and options, metric and chart labels — in NATURAL THAI, the way a Thai engineer actually writes, NOT a word-for-word rendering of an English sentence (if a Thai phrase only makes sense next to the English it came from, it is the wrong phrase). Do NOT translate technical terms: API and type names, tool names, flags, file paths, identifiers, and established jargon (race, guard, fan-out, diff, permission, …) stay in English inside the Thai sentence. The machine-parsed parts stay English too, or the document mis-parses: the fence names (`review:walkthrough`, `review:finding`, …), the field keys (`title:`, `severity:`, `steps:`, `at:`, `say:`, `question:`, `options:`, `recommended:`, `kind:`, `data:`), and the severity values `blocking` / `non-blocking` / `suggestion`. The `title` argument is the ONE exception: keep it short and in English, because it becomes the bundle's directory name on disk.",
            "inputSchema": {
                "type": "object",
                "properties": {
...
";
    ...
    fn bus_tools_include_the_review_board_trio() {
        ...
        // carry the never-unsolicited guard and the concise-reading contract.
        let new_tool = tools
            .as_array()
            .unwrap()
            .iter()
        ...
        assert!(description.contains("Make the Board a SKIM, not an essay"));
        assert!(description.contains("3–5 short bullets"));
        assert!(description.contains("at most three short sentences"));
        assert!(description.contains("Omit process narration"));
