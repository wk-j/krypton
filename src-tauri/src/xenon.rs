...
use crate::git;
use crate::review_excerpt::{collect_review_excerpts, created_ms_from_review_md};
...
fn collect_bundles(
    ...
    for dir in bundle_dirs(&base, depth) {
        ...
        if kind == "review" {
            attach_review_excerpts(cwd, &mut resource);
        }
        out.push(resource);
    }
    Ok(out)
...
/// Bake spec-217 excerpts into the published revision (spec 230).
/// Generated bytes only — nothing is written back under `.krypton/reviews/`.
fn attach_review_excerpts(cwd: &Path, resource: &mut LocalResource) {
    let Some(path) = resource.sources.get("review.md") else {
        return;
    };
    let Ok(text) = std::fs::read_to_string(path) else {
        return;
    };
    let sidecar = collect_review_excerpts(cwd, &text, created_ms_from_review_md(&text));
    let Ok(bytes) = serde_json::to_vec(&sidecar) else {
        return;
    };
    resource.inline.insert("excerpts.json".to_string(), bytes);
}
...
mod tests {
    ...
    fn bundle_collection_finds_reviews_at_depth_one() {
        ...
        assert!(
            found[0].inline.contains_key("excerpts.json"),
            "spec 230 sidecar is generated at collect, not written to disk"
        );
        assert!(!bundle.join("excerpts.json").exists());
