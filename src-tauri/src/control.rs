...
/// Stable capability registry shared by HTTP and trusted internal adapters.
pub const ADVERTISED_OPERATIONS: &[&str] = &[
    "harness.list",
    "harness.create",
    "lane.list",
    "lane.spawn",
...
/// Trusted request origin generated by Rust. Public control API callers cannot
/// supply this field; Telegram metadata is attached only after allowlist checks.
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct ControlCaller {
    pub source: String,
    #[serde(skip_serializing_if = "Option::is_none")]
    pub telegram: Option<TelegramCaller>,
}

impl ControlCaller {
    pub fn control_api() -> Self {
        Self {
            source: "control_api".to_string(),
            telegram: None,
        }
    }

    pub fn telegram(caller: TelegramCaller) -> Self {
        Self {
...
pub struct ControlServer {
    ...
    events: broadcast::Sender<ControlStreamEvent>,
...
impl ControlServer {
    ...
    /// such as Telegram must not create a second harness observer path.
    pub fn subscribe(&self) -> broadcast::Receiver<ControlStreamEvent> {
        self.events.subscribe()
    }

    /// Shared frontend round trip used by HTTP and trusted internal adapters.
            ...
            loop {
                match rx.recv().await {
                    Ok(frame) => {
                        ...
                        let event = SseEvent::default().event(kind).data(data);
...
async fn operation(
    ...
    let reply = state
        .server
        .dispatch(&state.app, request, ControlCaller::control_api())
        .await;
    let status = if reply
        .error
        .as_ref()
        .is_some_and(|error| error.code == "invalid_request")
    {
...
mod tests {
    ...
    fn publish_assigns_monotonic_seq() {
        ...
        let seqs: Vec<u64> = (0..3).map(|_| rx.try_recv().expect("frame").seq).collect();
