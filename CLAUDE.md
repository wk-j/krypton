...
- **`/cyberpunk-aesthetic`** — Complete reference for the current cyberpunk UI style: glow recipes, scanline patterns, window chrome structure, animation patterns, theme integration pipeline, CSS variable namespace, and component checklist. Load when adding new UI components, building alternative style modes, or verifying visual consistency.
- **`/retro-futurism-ui`** — Design and build Retro-Futurism UI blending 1930s–1970s machine-age aesthetics with modern web tech. Covers 5 subgenres (Raygun Gothic, Atompunk, NASA Mission Control, Cassette Futurism, Soviet Cosmism), era-accurate palettes, typography stacks, shape grammar, implementation patterns, and animation library. Trigger on "retro-futuristic", "atompunk", "Fallout terminal", "NASA panel", "art deco tech", "cassette futurism", or similar.
