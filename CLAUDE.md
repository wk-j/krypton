...
- **ACP Harness Peering** (`src/acp/lane-bus.ts`, `lane-inbox.ts`, `inter-lane.ts`, `harness-directory.ts`) — user-directed inter-lane messaging. `LaneBus` emits `lane:status` events on every status mutation (centralized via `setLaneStatus()`). `InterLaneCoordinator` queues envelopes in per-lane inboxes, drains them on the next `idle` transition by injecting programmatic user-turns via `enqueueSystemPrompt()`. Lane status `awaiting_peer` blocks the composer until `#cancel` clears it. MCP tools `peer_send` + `peer_list` on the `krypton-harness-bus` server (renamed memory server). **Memory is handoff-only (spec 165):** the `memory_set`/`memory_get`/`memory_list` tools are scoped to the `#handoff`/`#resume` handoff store — NOT an ambient shared scratchpad. `renderPromptMemoryPacket` does not advertise them on any turn (only the `#handoff`/`#resume` prompts name them); the tool descriptions frame them as a handoff document with a stale-snapshot caveat (verify against the live repo before acting). Cross-lane *read* is retained (read any lane's handoff, write only your own). **Cross-harness peering (spec 141):** lane `displayName` numbering comes from a process-wide, never-recycled counter in the `HarnessDirectory` singleton, so names are globally unique and a bare `displayName` addresses any lane in any open harness view (even across projects). The directory routes a cross-view `peer_send` entirely in-process — the recipient side runs on the target view's coordinator (`acceptInbound`), the sender side on the sender's (`recordOutbound`); `peer_list` folds in foreign peers tagged `local:false` carrying their `cwd`. The `#review` command (spec 145) is agent-orchestrated: it collects a review subject (working diff or a design doc) and injects one prompt directing the convening lane to `peer_send` it to every reviewer in one turn, then synthesize the replies — it rides `peer_send` (same-project, shared worktree) and has no bespoke MCP review tools or review card (the spec-112 channel was removed). The **review quality matrix** (spec 146, `src/acp/review-quality.ts`) accumulates a summary-only history of `#review` rounds per authoring lane: after synthesizing, the convening lane self-reports via the default-on `review_outcome` MCP tool `{ blockers, warnings, reviewer_count, subject_label }`, which the view records in an in-memory `ReviewQualityStore` (session-only, dropped on dispose). It is an **observation, not a score** (ADR-0004): raw counts only, no grade/verdict/diff-size/ranking. Surfaced like attention triage — a neutral `N reviews` depth indicator in the workspace footer (ViewBus `review:quality`, distinct from the attention gauge) plus a summon-on-demand overlay (`Leader '`, j/k to switch lane, read-only). **Diff review priority (spec 160):** the authoring lane self-reports a per-change reading-order hint via the default-on `mark_review_priority { ranges }` MCP tool (new-side line anchors, `high`/`routine` only — `normal` is the unreported default); `AcpHarnessView` stores the latest report per lane in a `ReviewPriorityStore` (spec 162, `src/acp/review-priority-store.ts`) and the Diff Window pulls a merged snapshot on open + each auto-refresh (compositor broker → `diff.review-priority` control op) to **fold `routine` hunks in place** (always expandable, `Enter`) and **mark + navigate `high` hunks** (`}`/`{`). Never hides or reorders; advisory only (ADR-0009); silence degrades to the full diff. A **cross-file priority panel** (`p`) docks right of the diff and **live-previews** every reported range as you walk it with `j`/`k` (no `Enter` — `high` grouped first, then `routine`; same-file scroll is instant, file switches debounced ~80ms; `Esc` restores the pre-open position, `Enter`/`q`/`p` keep the previewed one). **Review priority roll-up (spec 162):** the same store also drives a harness-level surface mirroring the review quality matrix — a neutral `N priority` (high-count) footer indicator (ViewBus `review:priority`, hidden at zero, never coloured) plus a summon-on-demand read-only overlay (`Leader /`, j/k to switch lane) that lists each lane's reported ranges grouped read-first/routine; acting on a range still happens in the Diff Window's `p` panel. **Lane monitor (live web dashboard, spec 168):** a `HarnessTelemetryPublisher` (`src/acp/harness-telemetry.ts`) subscribes to `LaneBus` + the triage/review stores and, on real source changes only (300ms debounce, no periodic tick), folds them into a harness-wide `TelemetrySnapshot` (lanes with exact `HarnessLaneStatus`, `observedTurns` proxy, attention/review/priority counts, foreign peers, a ≤14 closed-`EventKind` event ring) and `invoke`s `acp_publish_telemetry`; the built-in loopback server (`hook_server.rs`) caches it per-harness and serves a **fixed standalone page** `GET /dashboard` (`include_str!` of `src/acp/artifact-dashboard.html`) + a token-free `GET /telemetry` (`{ harnesses: [...] }`). The page polls `/telemetry` ~1s → attention gauge + per-lane cards (status pill, attn/rev/pri/peer chips, activity-pulse canvas for active lanes) + client-templated event stream. Stable bookmarkable URL via a **fixed default port 8765** (`config.rs`, ephemeral fallback on bind conflict): `http://127.0.0.1:8765/dashboard`, opened in an external browser anytime via the app command (`Leader Shift+L` / command palette `openDashboard`) or the harness `#dashboard` command. Read-only, no token, no `tok/s`, no radar. **Resource status (spec 169):** each `TelemetryLane` also carries the current CPU%/RSS/proc_count sample (folded from the spec-91 `metricsBySession` via a `metricsFor` accessor mapping `lane.id → client.sessionId → metrics`); the per-lane card carries **two distinct visuals** that answer different questions: the spec-168 **synthetic activity heartbeat** (`.beat` 22px canvas, `drawBeat`/`beatBuf`) — a status-coloured rhythm (busy spikes, needs-permission/awaiting-peer breathe, idle/stopped flatline) that is NOT real data, only a glance-level "is this lane alive / what's it doing" — **plus** a separate **live CPU sparkline** (`.pulse` 40px canvas, `drawCpuSpark`/`buf`, heat-coloured green/yellow/red, real per-lane CPU% accumulated client-side from successive polls) and a memory bar. The two coexist: heartbeat = "active?", sparkline = "how loaded?". To keep the chart honest the metrics poll publishes at its 2 s cadence **only while ≥1 lane is active** (`anyLaneActive()` gate — the one deliberate deviation from the no-periodic-tick rule; an idle harness still makes zero periodic pushes). `TELEMETRY_SCHEMA_VERSION` bumped 1→2 in lockstep with the page's `SCHEMA_VERSION`; no Rust changes (snapshot is a value-opaque `serde_json::Value`). **Artifact gallery (spec 170):** a read-only loopback sibling of the dashboard — `GET /gallery` serves a fixed standalone page (`src/acp/artifact-gallery.html`) and `GET /artifacts` returns token-free JSON `{ harnesses: [{ harnessId, artifacts: [{ id, laneLabel, title, state, size, hash, tail, token }] }] }` (both pending + live) read straight from the in-memory artifact registry; the page polls ~1s and renders a harness-grouped card grid with one-click open to `/artifact/<token>`. The gallery reads **only the in-memory registry** = active set (disposed harnesses delist automatically). Opened via `compositor.openGallery()` / command palette `gallery.open` / the `#gallery` harness command (mirrors the dashboard wiring; no keybinding — the dashboard's `Leader Shift+L` slot is taken). **Artifact files are never deleted** (spec 170 resolved fork): all four file-deletion paths in `hook_server.rs` were removed (`dispose_harness_artifacts` no longer `remove_dir_all`s; `sweep_stale_artifacts` fully deleted along with `init_harness_artifacts`'s `live_harness_ids` param; both pending-cancel paths dropped their `remove_file`) — disk is append-only history, the in-memory registry is the active-set view; only registry/token/telemetry cleanup remains. Documented trade-offs: `/artifacts` returns the feedback token (so artifacts are enumerable over loopback — the token is also a turn-injection cap via `POST /artifact/feedback/<token>`; shipped as Option A, consistent with the `/telemetry` loopback posture); on-disk growth is now unbounded with no in-app GC (manual `rm -rf .krypton/artifacts/` only); persisted files are not browsable after restart (no disk rehydration — by design). See `docs/106-inter-lane-messaging.md`, `docs/141-cross-harness-peering.md`, `docs/145-harness-design-review-panel.md`, `docs/146-review-quality-matrix.md`, `docs/160-diff-review-priority.md`, `docs/162-review-priority-roll-up.md`, `docs/168-harness-lane-monitor.md`, `docs/169-dashboard-resource-status.md`, `docs/170-artifact-gallery-endpoint.md`.
