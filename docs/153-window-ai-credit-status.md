...
| Codex | account-level `5h`, `week` | Model-specific buckets are not displayed in either the footer or detailed `Leader $` view |
...
- Codex renders only the account-level `5h` and `week` windows in both the footer
  and detailed `Leader $` view, matching the compact status in ChatGPT. Named
  model buckets such as Spark and GPT Reserve are not displayed.
