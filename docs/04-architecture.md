...
3. **Layout engine** — compute window positions relative to the full-screen workspace. Supports five modes: **Grid** (balanced auto-tile), **Focus** (focused window on left at full height, remaining windows stacked on right), **Depth** (Z-stack card deck), **Scroll** (niri-style columns on a horizontal strip; spec 241), and **Stage** (one large active window plus up to five scaled live previews on a left shelf; spec 245). Stage gives every window the same unscaled base bounds and changes only visual transforms during focus switching, so xterm/PTY dimensions remain stable.
...
19. **Window AI credit status** — `src/usage-store.ts` is the shared, ref-counted frontend owner for Claude/Codex/Copilot/Cursor subscription usage polling. A visible content view optionally declares its providers through `ContentView.getUsageProviders()`; ACP Harness publishes the deduplicated union of its lanes and signals membership changes. The compositor subscribes only the active tab's focused pane and renders quota windows into the left side of that window's `.krypton-window__footer`, while transient notifications remain right-aligned — and are genuinely transient: the notification control decays back to empty (`.krypton-notif--empty`, `display: none`) ~9s after its last message instead of holding the rail for the rest of the session. See `docs/153-window-ai-credit-status.md` and `docs/40-notification-overlay.md`.
20. **Window status bar lane strip** — the same per-window footer also carries one two-letter drop cap per lane of the ACP Harness in that window's focused pane, with the active lane magnified and showing a small trailing number (`CL²` next to `KR ypton`). Its full name remains in the tooltip and accessible label. It reuses the spec-153 contract exactly: `ContentView.getLaneMarks()` + `onLaneMarksChange()`, followed through one `syncWindowFooter(win)` that drives both readouts and is torn down in `closeWindow()`. Per-window by design — each window answers for its own harness, so the workspace footer stays out of it. Cap, drop-cap split, repaint key, and a11y label are pure functions in `src/window-footer-lanes.ts`. See `docs/218-window-lane-strip.md`. Immediately **left of the strip** sits the spec-219 project badge — the focused view's project name with its first two characters magnified (a drop cap) so tiled windows are told apart while scanning, paired with the lane drop cap so project and active lane read as one glance target at the rail's right end, derived from the already-existing `ContentView.getWorkingDirectory()` and rendered by the same `syncWindowFooter(win)`. Name extraction, `~`-abbreviation and the head/tail split are pure functions in `src/window-footer-project.ts`. See `docs/219-window-project-badge.md`. **Between the two** sits the spec-220 diff stat — the repo's uncommitted `+added -removed` against `HEAD`, magnified to the same level so project, volume and lane read as one oversized phrase. It is the rail's one *subscribing* readout of the three: `src/diff-stat-store.ts` polls per **repo root** (ref-counted, single-flight, stopped while the app is hidden), so several windows on one project share one git call, and `attachToBus` pushes it at `harness:lane-idle` so the number lands the moment a lane's turn ends. Totals come from `git.rs::working_diff_stat()` in one `--numstat` — never a diff — and the formatting is a pure function in `src/window-footer-diff-stat.ts`. See `docs/220-window-diff-stat.md` and `docs/adr/0020-diff-stat-polls-diff-window-does-not.md`.
...
25. **ACP Harness view** — `AcpHarnessView` (`src/acp/acp-harness-view.ts`) opens multiple `AcpClient` lanes in one content tab for the focused working directory. It renders a read-only lane dashboard plus an input-only command center, shows the project cwd and current Git branch in the composer status line, routes prompts only to the active lane, handles per-lane permission prompts, stages pasted/dropped/global-captured images in the active lane composer, sends staged images as ACP image blocks with base64 data and local file URIs, and creates an MCP memory store on the existing localhost hook server. Direct composer insertions spawn a phosphor afterimage of the inserted graphemes that ignites, drifts upward 3px, and dissolves (spec 252): overlay copies of the letters themselves, not an expanding ellipse. The overlay survives composer HTML rebuilds so fast typing trails; the existing inline caret is unchanged. `[acp_harness] composer_bloom`, `composer_bloom_ms`, and `composer_bloom_trail` configure it; Reduce Motion or `composer_bloom = false` drops it. This acknowledgement has no ACP, PTY, persisted lane-state, or custom-cursor role. The command center also owns one opt-in Web Speech recognition session: `src/acp/harness-dictation.ts` probes the host webview, reconstructs final/interim results, and inserts recognized text into the saved active-lane draft without changing ACP transport or submitting automatically. `Cmd+D` / the `MIC` control listen in English (`en-US`); `Cmd+Shift+D` listens in Thai (`th-TH`). Capture is aborted on cancel, lane/view teardown, composer pre-emption, or focus leaving the Harness; macOS packaging declares microphone/speech purpose strings and the audio-input entitlement. Each lane receives a lane-scoped HTTP memory endpoint; agents create/update/delete/search/get memory through MCP tools while the human observes the current board. Harness hash commands provide lane lifecycle controls including `#cancel`, `#restart`, `#new`, `#new!`, `#mem clear`, and `#mcp`; `#panels [hide | show | toggle]` independently hides persistent auxiliary rails/docks without hiding transcript content, the composer, or inline approval surfaces. Fresh-session commands use client disposal and a lane spawn epoch to ignore late events from replaced subprocesses. Memory and panel visibility persist per project directory. Open via `Leader Y` or command palette. The active lane **right rail** can show a contextual **lane peek** for one inferred non-active lane plus optional **lane-pair activity heat** (active vs peeked; palette-controlled metric/window/detail). See `docs/72-acp-harness-view.md`, `docs/73-acp-harness-mcp-memory.md`, `docs/109-acp-contextual-lane-peek.md`, `docs/246-harness-dictation.md`, `docs/251-harness-panel-visibility-command.md`, and `docs/252-harness-composer-soft-bloom.md`.
...
28. **ACP Harness render performance** — high-volume ACP event streams are batched in `AcpHarnessView` with a `requestAnimationFrame`-backed `scheduleLaneRender()` / `scheduleStreamingBodyOnly()` so repeated message/tool chunks coalesce into one active-lane pass per frame. Lightweight `available_commands` and `mode_update` events patch only their affected composer/header surfaces. `stop` and MCP-stat refreshes do not rebuild the dashboard. Assistant transcript items cache parsed/highlighted markdown on the row object, and pretext transcript rows cache measured line layouts by text, width, font, and line-height — a later layout pass skips rows whose DOM already matches that cache. Composer key edits pin a stuck transcript in the same turn so a swallowed scroll event cannot paint the 14px per-keystroke jump (spec 114 rev 14). Streaming tool-output updates do not replay a smooth-follow glide when the transcript bottom did not move (spec 114 rev 15). See `docs/94-acp-harness-render-performance.md` and `docs/114-acp-harness-streaming-perf.md`.
...
33. **Quick Overview dialog (spec 210)** — `QuickOverview` (`src/quick-overview.ts`) is the one global *read-only* modal for showing content, as opposed to the pickers/composers (command palette, quick file search, prompt dialog). It renders a pluggable `OverviewSource` — `{ title, meta, load(signal), actions }` returning an `OverviewBody` of `code` / `markdown` / `notice` / `element` — and owns the shell, the single scroll container, cancellation (`AbortController` per open), and the keyboard (`j`/`k`, `Ctrl+D`/`Ctrl+U`, `g`/`G`, `y`, `Esc`; source actions are dispatched first so a source can override a key). Editing is never inline: a source action escalates (`Enter`). The first consumer is hint mode's `filepath` rule via `createFilePeekSource()` (`src/file-peek-source.ts`), which resolves the path as written (absolute / `~` / `file://` / relative to `getFocusedWorkingDirectory()`), reads it with `read_file`, classifies it with the shared helpers in `src/file-preview.ts` (also used by the file manager preview), and maps `Enter` back to the pre-210 openers — Helix tab, markdown viewer, or OS browser. `[hints] file_action = "editor"` restores direct opening. `InputRouter` owns the `Mode.QuickOverview` transition, including the hint→overview handoff. See `docs/210-quick-overview-dialog.md`.

34. **Review Board (spec 211)** — The surface where a lane *explains code* to the human and the human works through it. A `PaneContentType = 'review'` content window (`src/review-board/view.ts`) over a **lane-authored Markdown document plus typed fenced blocks** (`walkthrough`, `diff`, `finding`, `decision`, `chart`, `metrics`, `svg`), parsed by `src/review-board/parse.ts` (one `ReviewBlock` per top-level `marked` lexer token; a malformed or unknown `review:*` fence degrades to a diagnostic-carrying `markdown` block and is **never dropped**) and built into DOM by `src/review-board/render.ts` (markdown via the Markdown Viewer's `marked`+`highlight.js` stack, `diff` via `diff2html`, single-series accent-hued inline SVG for charts, allowlist-sanitized `svg`). Blocks are addressed by **stable content-addressed id** (`b<ordinal>-<fnv1a(raw)>`, `data-block-id`), never by CSS selector into rendered content — that is what lets a comment re-attach after the lane rewrites the document (`reattachBlockId` falls back from an exact id to a *unique* hash match). **Comprehension is the job, not defect-hunting**: prose plus a `walkthrough` is the required spine, findings/decisions are optional additions, and a Board with neither reads as `reference` rather than "0 unanswered" — never a score (ADR-0004). Keyboard: `n`/`N` block cursor, `}`/`{` next unanswered, `Tab`/`Shift+Tab` the guided walkthrough read (each step jumping the Diff Window via `DiffContentView.revealLocation`, falling back to a reader when the path is not in the diff), `a`/`x` triage, `1`…`9` decisions, `c` comment, `o` Review Map, `O` Overview, `[`/`]` chapter, `/` search, `s` send, `r` reload. **Spec 244 made it a chaptered reader**: chapters are derived from the document's own `H1`/`H2` headings (`ReviewDocument.sections`, derived state that is never serialized), the body renders one chapter at a time, and a 272px **Review Map** — `Overview`, the chapters with block/unanswered counts, `Open items`, `Full document` — is a permanent column at ≥1180px pane width and an on-demand overlay below it (driven by the view's own `ResizeObserver`, because `ContentView.onResize` is declared but never called). `Overview` renders no authored block and reports raw counts only, never a score. Render paths are split so a cursor move inside a chapter re-classes one element instead of rebuilding the document. **The result is a file, not session state**: `review_new`/`review_register`/`review_cancel` (`hook_server.rs`) clone spec 133's path-handoff into a durable gitignored bundle at `.krypton/reviews/<date>-<slug>/` (`review.md` + `response.md` + `assets/`), with issued-path write auto-approval widened from one file to the whole directory (`reviewWritePathMatches`, trailing-slash bounded so a sibling `-2` bundle cannot match). Answers **autosave** (debounced 400 ms → `write_review_response`; frontmatter is the source of truth, the body a generated rendering), so closing loses nothing and `s` is a separate act that routes `review.response` → `ReviewResponseQueue` (a third drain-on-idle queue beside specs 149/158) → a system turn. Bundles are **never swept** and are rediscovered by a directory walk (`list_review_bundles`), so `Leader Shift+R` → picker reopens a review from a previous app run intact; `#reviews` opens the read-only `/reviews` archive (spec 192's template, typed blocks re-rendered by a fence-aware post-pass over comrak). `#review`'s synthesis step now composes a Board instead of ending in turn text. See `docs/211-review-board.md`.

35. **Xenon publisher (specs 212/259)** — The one path by which harness-generated work leaves this machine. `src-tauri/src/xenon.rs` collects seven resource families: fixed-depth `review` and `analysis` bundles from the gitignored `.krypton/` tree; `artifact` HTML; repo `doc` markdown; rendered `daily` notes; in-memory `attention` flags; and confirmed `timeline` events. Timeline collection reuses `timeline::scan_project()`, publishes one event as `event.md` plus bounded chronology metadata, ignores pending/dismissed suggestions, and reports malformed event files as non-retryable per-item failures without aborting other kinds. All families use the same three-step content-addressed protocol against a **Xenon** server (its own repo at `~/Source/xenon`): `POST …/resources` returns only missing digests, `PUT /v1/blobs/<sha256>` uploads those bytes, and `POST …/commit` seals the revision. An identical re-push short-circuits on `unchanged`. Driven by `#push [--force] [<kind> [<slug>]]`; `#xenon timeline [<topic>]` opens Xenon's hosted chronology while local `#timeline` remains the authoritative browser. Publishing is **explicit, never ambient** (ADR-0016): a secret pre-scan blocks a resource, artifacts lose their loopback feedback capability before upload, retryable transport failures land in `.krypton/xenon-queue.json`, and the bearer token lives in the OS credential vault rather than TOML. Xenon is a sink only and never creates or confirms a Krypton timeline event. See `docs/212-xenon-resource-server.md`, `docs/259-timeline-xenon-publishing.md`, and ADR-0016.

36. **Backend link indicator (spec 213)** — The workspace footer's answer to "is the off-machine backend actually alive?". `xenon_status` reports *configuration* (it reads the TOML and asks the credential vault whether a token exists) and never touches the network, so a server that is down, a revoked token, and a healthy link were indistinguishable until a `#push` failed. `src/backend-link.ts` schedules a periodic `xenon_probe` — one authenticated `GET /v1/projects`, which distinguishes *unreachable* (transport error), *unauthorized* (401/403), and *linked* (2xx) in a single request; the server's `/healthz` cannot see the bearer token, so probing it would report green while a revoked credential blocked every push. The result is published as `system:backend-link` on the ViewBus and rendered by `WorkspaceFooter.renderLink()` as a fourth `sourceId`-keyed segment at the outer edge of the right cluster. Ticks are skipped while the document is hidden and fire once on re-show; `⌘P X` re-probes on demand; a completed `#push` publishes the same signal (stronger evidence, no extra request). The probe path caches the token and the derived project slug for the process lifetime — the uncached `load_token` stays on the push path so a publish never acts on a stale credential. Coloured only when faulted, and hidden entirely when `[xenon].enabled = false` (ADR-0017). See `docs/213-backend-link-indicator.md`.
...
| **QuickOverview** | Selecting a `filepath` hint (or any future source-based peek) | Read-only Quick Overview dialog renders an `OverviewSource` (code / markdown / notice / element). `j`/`k`, `Ctrl+D`/`Ctrl+U`, `g`/`G` scroll; `y` copies the path; `Enter` runs the source's escalation | `Escape`, or a source action that closes |
...
- `Cmd+Shift+<` / `Cmd+Shift+>` — Cycle focus counterclockwise / clockwise through visible Grid and Focus windows; other layouts retain their native linear or stack order
...
[Current mode?]
  ...
  +-- QuickOverview -> Scroll the peeked content / run a source action
...
The project timeline (spec 253) is another project-local surface, but it is an
append-only provenance ledger rather than mutable work state. `timeline.rs`
resolves the project from the registered Harness id, confines one Markdown file
per event to `.krypton/timeline/events/`, validates bounded schema-1 records,
...
project-backed `timeline_suggest` MCP tool lets the lane already doing the work
write one bounded candidate to `.krypton/timeline/pending/`; no sidecar model or
post-turn request runs. `timeline.rs` owns pending/dismissed files plus the
atomic `.krypton/timeline/settings.json` switch. HookServer emits
`acp-timeline-suggestion` after a successful write, while `AcpHarnessView` owns
the passive count badge and the human review sheet. Confirmation alone creates
`events/<reserved-id>.md` and preserves suggestion evidence/provenance;
...
`event.md` and enough structured metadata for Xenon to sort by occurrence time,
group topics, derive supersession, and link relations. `#xenon timeline [<topic>]`
opens that hosted read model. Neither recording nor confirmation triggers a push.
