...
35. **Xenon publisher (spec 212)** — The one path by which harness-generated work leaves this machine. `src-tauri/src/xenon.rs` collects the five resource families (`review` and `analysis` bundles by a fixed-depth `read_dir` walk of the gitignored `.krypton/` tree — an ignore-aware walker would filter the whole subtree out; `artifact` HTML; repo `doc` markdown; and `attention` flags, which have no on-disk form and are handed over from the frontend's `AttentionTriageStore`), then pushes each with the three-step content-addressed protocol against a **Xenon** server (its own repo at `~/Source/xenon`): `POST …/resources` returns only the digests the server is `missing`, `PUT /v1/blobs/<sha256>` uploads exactly those, and `POST …/commit` seals the revision. An identical re-push short-circuits on `unchanged` without transferring a byte. Driven by `#push [--force] [<kind> [<slug>]]` and `#xenon` (`src/acp/xenon-push.ts` owns the pure grammar and report formatting), and by `xenon.push`/`xenon.status` on `/control/v1` so kryptonctl, Telegram, and Raycast reach the same code. Publishing is **explicit, never ambient** (ADR-0016): a secret pre-scan blocks a resource instead of leaking it, artifacts have their spec-149 loopback feedback capability stripped before upload, retryable transport failures land in `.krypton/xenon-queue.json` while auth failures deliberately do **not** (queuing a bad credential would retry it forever), and the bearer token lives in the OS credential vault (`com.krypton.xenon`, keyed by `base_url`) rather than in TOML — the ADR-0015 pattern. Xenon is a sink only: it stores and serves, and has no channel back into a running harness, so remote control stays exclusively with `/control/v1` (ADR-0005/0007). See `docs/212-xenon-resource-server.md` and `docs/adr/0016-generated-resources-publish-to-xenon.md`.
