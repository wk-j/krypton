...
- Built-in harness MCP permissions do not show this banner. The harness automatically chooses the first allow option for the built-in tools — memory (`memory_set`, `memory_get`, `memory_list`), peer (`peer_send`, `peer_list`), and attention (`attention_flag`, `attention_resolve`) — then updates the original permission card to `auto-allowed` with a visible harness-rule reason. (Attention is default-on per spec 130; auto-allow also preserves its non-blocking contract.)
