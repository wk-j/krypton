...
- `#timeline conflicts` opens a keyboard-first sheet for decisions that may contradict each other (spec 266). `j`/`k` moves between pairs (still-to-review first), and the two events sit side by side with who decided, when, source, and reason. `1` confirmed conflict, `2` not a conflict, `3` insufficient evidence, `4` resolved open a review form with a required rationale; `resolved` also needs the event that ended it or a source reference; `Cmd/Ctrl+Enter` saves and `Esc` backs out. Reviews are appended, never overwritten, and are always attributed to the local user. `n` picks two events from any topic to propose a pair with a reason; `s` starts a TypeSafe screening scan when `[typesafe.timeline_conflicts] mode = "suggest"`. The status line never reads "no conflicts" after a partial or missing scan. In the browser Timeline, rows in a pair still to review (or confirmed and open) get a coloured chip on their time cell only, `made_by` is always shown as recorded, the detail links to the other side and to superseding/superseded events, `p` jumps to the paired event, and `?topic=<id>&event=<id>` deep-links one event. The page stays read-only. See `docs/266-timeline-decision-trace-and-conflicts.md`.
