...
- `#timeline conflicts [<topic>]` hands conflict review to the lane agent (spec 267): it reads `timeline_conflict_list`, compares only events not yet checked (one topic, or at most five unchecked topics per run), records each finding with `timeline_conflict_record` (`confirmed`, `resolved`, `insufficient_evidence`, or `dismissed` to close an open pair; attributed to the lane), and marks what it compared with `timeline_conflict_checked`. The agent also does this after every `timeline_record`. There is no sheet or verdict form. In the browser Timeline, rows in a pair still to review (or confirmed and open) get a coloured chip on their time cell only, `made_by` is always shown as recorded, the detail links to the other side and to superseding/superseded events, `p` jumps to the paired event, and `?topic=<id>&event=<id>` deep-links one event. Superseded rows are hidden by default; the button or `h` shows them, and a deep link reveals its target. The page stays read-only. See `docs/266-timeline-decision-trace-and-conflicts.md` and `docs/267-timeline-conflicts-by-agent.md`.
