...
- When the current human message explicitly says to record, remember, persist, or add something to the timeline, a local MCP-capable lane calls `timeline_record` directly. That sentence is the confirmation: no capture/review/permission modal opens. The final event stores the exact `instruction_excerpt`, attributes the recorder lane, deduplicates retries, and promotes a matching pending suggestion under its existing ID. Agent-composed topic, summary, rationale, and impact use natural Thai; the authorizing instruction and provenance stay exact. The lane reports the returned event ID/path. This path is independent of `#timeline auto`; a lane must still use `timeline_suggest` when persistence was not explicitly requested. See `docs/255-natural-language-timeline-recording.md`.
