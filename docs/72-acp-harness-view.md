...
- **Activity ticker (spec 156 / 231)**: the live action (`edit` / `read` / `thinking…` / …) is no longer a composer segment. It paints a compact **action HUD** stack in the lane rail (`[data-slot="action"]`, stacked above the peek card), with a distinct instrument animation per kind — one card per busy lane in the same harness tab. `lane.activity` is still written as a field on the hot path; the 1 s composer tick, background lane renders, and the existing render paths patch the HUD in place (a new execute does not replay the entrance; remount only when labeled chrome appears or disappears). Thinking is omitted when the thought slot is already showing that lane's live thought; an in-flight tool still paints (tools beat thinking), and an omitted-thinking empty stack keeps the last tool card instead of starting the hide timer. Empty frames with no action at all hold the last HUD for 2s before the slot hides, so a true idle still dismisses the card. A busy peeked lane reuses the same HUD inside the 109 card (and drops the activity event row that would restate it) and is then skipped in the stack. The composer busy chip keeps verb / elapsed / queued only. See `docs/156-lane-activity-ticker.md` and `docs/231-lane-peek-action-hud.md`.
