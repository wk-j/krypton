...
| `src/compositor.ts` | Add `openAcpHarnessView()` using `getFocusedCwd()`. The harness *replaces the launching terminal tab*: open it as a content tab in the same window via `createContentTab()`, then close the terminal tab it launched from (`closeTabByIndex`) so the harness slides into its place — no new window, sibling tabs and other windows untouched. Wire the close callback to `closeTab()`. Fallback: when there is no focused window (empty workspace), open the harness in its own window via `createContentWindow()` with a `closeWindow(harnessWindowId)` close callback. |
...
interface HarnessLane {
  ...
  lastTurnTokens: CacheTokenTuple | null; // coherent cache-rate tuple (spec 249)
...
Default roster: **none.** The harness boots empty (`lanes = []`). On first open, instead of auto-spawning a default lane, `start()` opens the lane picker automatically (when at least one supported backend is installed) so the user chooses which ACP backend to start; dismissing the picker with `Esc`/`q` leaves the harness empty with the `press Cmd+P then + to add a lane` hint. Thereafter the user spawns more lanes on demand via the lane-picker leader key (`Cmd+P → +`). Gemini remains available in the standalone ACP agent view but is intentionally excluded from the multi-lane harness picker, session picker, and backend-scoped directives. The picker order is Claude, Codex, Grok, OpenCode, Pi, then remaining installed backends by id (`HARNESS_BACKEND_ORDER` in `src/acp/harness-lane-identity.ts`). Backend display names are mapped through `BACKEND_LABELS` (`claude` → "Claude", `codex` → "Codex", `grok` → "Grok", `opencode` → "OpenCode", `pi-acp` → "Pi", `droid` → "Droid", `cursor` → "Cursor", `junie` → "Junie", `omp` → "OMP"); unknown backend ids fall back to a capitalized form. See `docs/92-acp-lane-picker.md` for the picker UI and lane lifecycle. Earlier versions auto-spawned every installed backend on view open — that path was removed because it forked long-running ACP subprocesses before the user typed anything.
...
Krypton auto-allows ACP permission prompts only for its exact built-in Harness MCP allowlist on the `krypton-harness-memory` / `krypton-harness-bus` endpoint. That list includes memory/peer/attention/review bookkeeping, bounded ticket progress, pending-only `timeline_suggest`, and `timeline_record` when the current human prompt explicitly requests persistence. The direct tool stores the exact authorizing instruction and returns the event ID/path in the same turn; it does not make unsolicited capture authoritative. Memory is lane-scoped and tab-local; peering remains user-directed by the prompt instructions. Auto-allowed requests still render a structured permission card in the lane transcript with the matched harness rule so the decision is auditable. Other ACP tool permission requests still use the normal composer pre-empt flow.

Spec 133 adds a **separate** auto-approval mechanism for HTML-artifact file writes. Because the write is performed by a backend-native filesystem tool (not the harness MCP server), it **cannot** key off the spec-96 built-in-server marker. Instead it matches the write target's canonical path against an issued artifact path in the registry for the same `harnessId + laneId + artifactId` (state `pending` or `registered_live`). The auto-approved write card is redacted to path + bytes + hash. See `docs/133-harness-html-artifacts.md` and `docs/96-acp-built-in-memory-auto-approval.md`.
...
#### User-triggered handoff (spec 139)

Cross-session handoff reuses the lane's existing `memory_set` document but is **user-triggered, not always-on** — unlike attention-triage or the lane-context stub, an always-on handoff would push proactive memory reads/writes every session (real token cost), so the cost is paid only on an explicit command. Two composer hash-commands inject a one-shot instruction turn into the active lane via `enqueueSystemPrompt()`:

- `#handoff` — instructs the lane to write/refresh its `memory_set` document into a resume-ready shape (what's done, current state, next steps, open questions; reference artifacts by path; never write secrets, since `memory_set` is not redacted; overwrite, ≤ 8000 chars).
- `#resume` — instructs the lane to call `memory_get { lane: "<self>" }`, load its prior-session handoff, and continue (start fresh if empty).

Both guard with a `flashChip` no-op when memory is unavailable (`harnessMemoryId` unset) or the lane is mid-work (`busy` / `needs_permission` / `starting` → `#cancel first`); they **are** allowed while `awaiting_peer`, matching the soft-awaiting model (spec 116) and the `enqueueSystemPrompt()` contract — a lane waiting on peer mail may still do user-directed work, and the coordinator restores the peer wait afterward. No always-on stub line, no per-lane resume flag, no per-turn cost. The lane-context stub (`renderPromptMemoryPacket`) is unchanged. See `docs/139-default-memory-handoff.md`.
...
8. Harness dispatches the prompt to the active tab's lane only. While the turn is running, the composer chip shows `m:ss[ · N queued]` and updates once per second (spec 221: the lane name and the generic `running` verb are not repeated here — the lane head and the spinner one row below already say them). Cancel is `Ctrl+C` / `#cancel` (workspace footer `#cancel running` when the harness is focused); the lane head does not paint a cancel chip. A busy peeked lane shows a flat tool row; there is no rail action HUD (spec 231 removed). **If the active lane is busy or in `needs_permission`, the prompt is enqueued** into a per-lane FIFO `queuedPrompts` (text + frozen image snapshot + mention targets, capped at 10) instead of being discarded; one item drains on each idle transition (spec 136). The queue renders in a bottom-anchored right-rail slot (numbered rows, `▸` next marker, `→lane`/`img×N` tags) and is managed with `#unqueue [N]` / `#queue clear` / `#queue edit N`. Peer mail drained on idle takes precedence (the queue shows `held behind lane mail` until idle). When the lane transitions busy→idle with a non-empty draft **and an empty queue**, the chip flashes `lane idle — Enter to send`; dispatch remains manual.
...
11. Permission requests for built-in harness memory/peer MCP tools are auto-allowed and update a structured permission card with `auto-allowed` plus the matched rule (`matched harness memory auto-allow rule` or `matched harness peer auto-allow rule`) without entering permission mode. Other permission requests move that lane to `needs_permission` and pre-empt that tab's composer with the options banner. The lane transcript holds the request detail and a cross-lane warning when `fileTouchMap[path]` exists from a different lane within the last 10 minutes. The user must switch to that tab to read context, then resolve via `a/A/r/R/Esc`. `A`/`R` apply only for the current `session/prompt` turn; both flags clear when that turn returns.
12. `Ctrl+C` (or `#cancel`) in the composer cancels the active lane only **and clears its prompt queue**. `#new` starts a fresh active-lane session after awaiting old-client disposal; `#new!` also clears the active lane's persisted memory when memory is available; `#mem clear` clears memory without replacing the session. `#unqueue [N]` removes the last (or 1-indexed) queued prompt; `#queue clear` empties the queue without cancelling the running turn; `#queue edit N` pops item N back into the composer to edit and re-send (spec 136). `#handoff` / `#resume` inject a one-shot instruction so the active lane writes or reads its `memory_set` handoff document — user-triggered cross-session handoff, no always-on cost (spec 139). `#draw <request>` injects a one-shot tldraw Offline workflow: the lane uses its existing tools and permission policy to discover the running app, read its version-matched `/readme`, pick the target document (the focused one, or a named one via the docs-listing filter — the substring filter must resolve to exactly one match or the lane stops to disambiguate), inspect it through the local agent API, then edit it along one of two branches — static content batches through `/api/doc/:id/exec`; durable or interactive behavior (clickable UI, animation loops, custom shapes) goes through the app's document-script workspace (`script-workspace` → read the matching `api.recipes` entry → edit only app-declared `editable` script files, extending any existing script → success only when `script-status` reports `state: "applied"`) — and finally verifies records plus a screenshot. If a loopback request fails while `server.json` exists, the prompt requires a listener check and a permitted-path retry before the lane may report the app as not running — a sandboxed lane must not mistake its own network restriction for a dead app (issue #11). The command adds no MCP tool or lane state; Krypton never stores the app token, and the packed `.tldraw` archive plus `appOwned` files stay untouchable — the app's watcher alone embeds script edits into the document (specs 196/197).
...
<div class="acp-harness">
  ...
  <div class="acp-harness__dashboard">
    ...
    <div class="acp-harness__lane acp-harness__lane--collapsed acp-harness__lane--permission">
      <header class="acp-harness__lane-head">[4] ! Droid-1 perm: write src/…</header>
    </div>
  </div>
  ...
  <div class="acp-harness__command-center">
    <nav class="acp-harness__tab-strip">
      ...
      <button class="acp-harness__tab">[3] OpenCode-1</button>
...
- The active lane's stats row shows the latest complete turn's prompt-cache hit rate as `cache N%` when the adapter reports a coherent input/cache tuple. The tooltip keeps read, write, and uncached-input counts. If input or cache-read is absent, the row keeps the raw read/write counters instead of inventing a percentage. `#usage` reports the day as a structured transcript card: cache hit + proportional input/cache composition, visible raw input/output/read/write metrics, per-model rows, and explicit cost/delivery notices. At narrow widths the four metrics become `2 × 2`, model rows stack their stats, and the cache bar moves below its percentage; recording-off and empty-day results remain plain sentences. The equivalent text summary remains on the transcript item for Live Assist and generic projections. `#daily` adds the same rate to each lane row.
- The active lane body shows the full per-lane transcript: user prompts, assistant text, thoughts, tool calls and summaries, plan updates, permission requests and resolutions, usage snapshots, and system rows. Tool rows show diff/output preview inline as monospace text. Kind chips reuse the spec 231 HUD accents (chip text + 1px border, not a filled pill). Dump body is foreground phosphor, not cyan; stdout/output/content/text drop the body text-shadow. Dump and inline-diff payload use the same `--krypton-font-size` as conversation text; kind chips, section labels, and timers stay chrome-scale. The group container has no background glow and no left rail (spec 235). Execute-tool output can render structured summaries for recognized command output: unified `git diff` output becomes compact file/hunk/add/delete rows, `git diff --stat` becomes per-file mini bars, and `git status --short` becomes status badges plus paths. A leading `exit: N` (Grok Bash `output_for_prompt`, or structured `exit_code`) peels off the dump and paints a quiet exit readout on the first OUTPUT/TERMINAL label — muted tabular number for 0, danger color for non-zero, no kind-chip frame — instead of a raw `exit: 0` line. Git-rich dumps (no section label) keep the readout as its own row. Grep/rg dumps (search kind, or execute wrapping `rg`/`grep`/`ag`/`ack`) split into path / line / text tokens when a majority of lines match, with optional hit spans from a quoted query. Line numbers right-align in a shared ≥4ch gutter so mixed digit counts and mixed path lengths stay in columns. Grok search wraps hits in a `<workspace_result>` envelope and groups them as a path header plus `line:text` rows; that envelope is stripped even when the line cap drops the close tag, so a miss is `No matches found`, not XML tags. Grok `read_file` line anchors (`1→`, `10→`) are stripped on read dumps. Grok `rawOutput.type` variants (`ReadFile`, `GrepSearch`, `ListDir`, `Bash`, `MCP`, …) are unpacked from their structured fields before that strip (spec 235); unknown types keep the generic key walker. Unknown output keeps the generic labeled text sections. Git-stat paths and unidiff file headers use the warning token, not cyan. Short provider/API failures that arrive as assistant text are classified when the stream seals and rewritten into `provider_error` cards with a headline, hint, retryability chip, and collapsed raw details; raw provider diagnostics are not markdown-rendered. Long assistant text truncates at ~12 lines with a `[…]` expand affordance. **Agent-emitted markdown images** (`![alt](path)`) are displayed inline: agents that generate images write files to disk and reference them by absolute filesystem path (e.g. Grok's `image_gen` → `~/.grok/sessions/<enc-dir>/<id>/images/N.jpg`), which the webview origin cannot fetch directly, so `resolveLocalImageSrcs()` rewrites local `<img src>` to a Tauri asset URL via `convertFileSrc()` at every markdown→DOM site (assetProtocol scope `$HOME/**`); a leading `/` is a true absolute path, relative paths resolve against the lane project dir, and remote/`data:`/already-resolved schemes pass through untouched. A failed load swaps in an `IMG BREACH // <path>` notice and the image is capped to the column width. The transcript scrolls within the lane body.
- **Thought rows** render dimmed at their full natural height. They have no per-message height clamp or inner scrollbar; the lane transcript owns scrolling, so both streaming and sealed reasoning remain fully visible in the conversation. A 5-bar effort meter under the `thought` label remains as a log-scaled depth cue. Repeated and leading/trailing blank lines in the thought body (Codex `\n\n` section breaks) are dropped on display without removing message text. **Veiled thinking:** providers that keep reasoning server-side (Claude Code on current Opus models) stream `thought_chunk`s whose text is empty — the model is thinking but no content ever reaches the client. While such a row streams, the body shows an animated placeholder (`thinking` + pulsing dots, opacity-only animation) instead of an empty block, and the effort meter is suppressed; if the row still has zero text when the stream seals (or the turn errors), it is dropped from the transcript entirely.
- A **contextual lane peek** can appear in the active lane transcript region for one non-active lane. It is a non-blocking, hideable top-right summary, not a global overview: it shows only the automatically selected lane's name, status, reason, and at most one compact payload (permission, peer, error, or recent activity). Selection prioritizes direct peer relations from the active lane, related permissions, non-active errors/permissions/inbox, then recent meaningful activity. Direct peer candidates (priority ≤30) clear a prior user dismiss before selection runs. `Esc` hides the peek only when no permission banner owns `Esc`; command palette actions can show/hide, cycle, unlock, or activate the peeked lane. No placeholder is shown when no non-active lane qualifies. Spec 216 adds a **sibling thought slot** (own card, same 320px column) under the peek. Hide / `Esc` dismiss the peek only. The thought slot shows the peeked lane’s thought when that card is up and has content, otherwise the active lane’s thought, and hides when neither has content. Spec 232 paints live thought as a catch-up teletype (caret + phosphor settle, no scan, no brackets); sealed thought stays GFM. **Lane-pair activity heat** (slice 109) adds a compact strip above the peek stat chips: active vs peeked bars for the selected metric/window, optional expanded rows (tools / tokens / peer / CPU), and palette actions to cycle metric (`auto`, tools, tokens, peer, process, alerts), cycle window (`30s`, `5m`, `session`), and toggle detail; click targets mirror palette behavior. Heat samples reuse the harness metrics poll and throttled transcript/usage snapshots (no extra timer). See `docs/109-acp-contextual-lane-peek.md`.
...
- **Rail entry anatomy (spec 125)** — each left-rail row is `dot · logo · head · meta`. The status dot (col 1) carries lane status; the backend logo (col 2, an inline-SVG monogram tinted per `backendId`) carries the adapter identity; the head row carries the lane display name plus peer/tool/ctx chips; and the meta row carries a role tag chip plus the directive title with the backend-label prefix stripped (presentation-only — storage and `peer_list` keep the full title). The role tag is derived from `directive.task` via keyword matching (`analysis` / `review` / `impl` / `plan` / `explore`); unmatched non-empty values display the raw lowercased `task` and pick a stable hash-bucket color; empty `task` displays the literal `custom`. Pending-clear adds an `--clearing` modifier to the tag (line-through, dashed border, dimmed). Unbound lanes render dot + logo + name + status-label meta with no tag. See `docs/125-lane-rail-disambiguation.md`.
- Permission requests appear inline in the transcript at the time they arrive as dense cards:

  ```text
  peer  peer_send  Claude-1                         auto-allowed
  matched harness peer auto-allow rule
  to_lane: Claude-1 · message: …
  ```
...
- **Transcript annotations (spec 240)** — select a span in an assistant reply, press `c`, type a note, Enter **adds** it to a local batch (highlight + notes rail; the lane is not contacted). Repeat, then `s` (transcript focus) or the rail **send N** control ships every unsent note to the **same lane** as one system turn (JSON `{ quote, heading, note }` framed as user data). Idle drain uses `TranscriptAnnotationQueue` immediately after the peer-mail coordinator. Unsent notes do not persist across restart. See `docs/240-harness-transcript-annotation.md`.
...
- **Direct-insertion feedback**: printable keys, `Shift+Enter`, plain-text paste, and `Ctrl+Y` keep the draft and existing inline caret fully immediate, then spawn overlay copies of the inserted graphemes as a cyan phosphor afterimage that ignites, drifts upward 3px, and dissolves (spec 252). Fast typing stacks a fading trail of letters — not an expanding ellipse; a paste flashes at most `composer_bloom_trail` visible graphemes (default 5) over `composer_bloom_ms` (default 320). The effect never replaces Krypton's cursor and does not run for Backspace/Delete, navigation, programmatic rewrites, dictation, lane switches, or status refreshes. System Reduce Motion or `composer_bloom = false` removes it. See `docs/252-harness-composer-soft-bloom.md`.
...
- **Git branch** (`⎇ feat/…`) sits on the same composer-meta row. It is probed at harness open (and again once `projectDir` resolves) and re-probed when a tool ends, a lane goes idle, or the user presses `r` in transcript command mode. A later `git switch` no longer leaves the previous ref on the chip; later probes keep the last label until the new one arrives so the row does not flash empty.
...
- Built-in harness MCP permissions do not show this banner. The harness automatically chooses the first allow option for the built-in tools — memory (`memory_set`, `memory_get`, `memory_list`), peer (`peer_send`, `peer_list`), attention (`attention_flag`, `attention_resolve`), and review (`review_outcome`) — then updates the original permission card to `auto-allowed` with a visible harness-rule reason. (Attention is default-on per spec 130; `review_outcome` is default-on per spec 146; auto-allow also preserves their non-blocking contract.)
- Contextual lane peek is controlled through command palette actions: `ACP: Show Lane Peek`, `ACP: Hide Lane Peek`, `ACP: Peek Next Lane`, `ACP: Peek Previous Lane`, `ACP: Activate Peeked Lane`, `ACP: Unlock Lane Peek`, plus heat controls when a peek is visible: `ACP: Cycle Peek Heat Metric`, `ACP: Cycle Peek Heat Window`, and `ACP: Toggle Peek Heat Detail`.
...
- If multiple permission requests are queued for the same lane, the composer stays in pre-empt mode and walks them in order; the lane transcript shows each request card and its in-place decision update. When `acceptAllForTurn` (or `rejectAllForTurn`) is set, the harness resolves subsequent requests automatically without re-displaying the banner; each auto-resolution still updates its permission card.
...
- The popup is bounded to the lower half of the Harness. Long result sets scroll inside the popup, keeping both the selected row and composer input visible above the workspace footer.
...
- Available commands:
  ...
  #panels     hide or restore persistent Harness panels
  #timeline   record, review suggestions or conflicts, browse, or trace local project history
  ```
...
- `#panels [hide | show | toggle]` controls one per-project presentation flag (bare `#panels` toggles). Hidden mode removes the active-lane auxiliary rail (plan, lane peek, live thought, and prompt queue), the Active Ticket dock, and the Zen lane-navigation rail while preserving their state. The lane header/stats, transcript, composer, inline permissions/questions/write reviews, and explicitly summoned modal tools remain available. Transcript thought rows are content rather than panel chrome; combine with Concise Mode when those should also disappear. See `docs/251-harness-panel-visibility-command.md`.

- `#timeline` opens the complete local timeline in the OS browser — a compact one-line-per-event list with sticky day headers. `j`/`k` moves selection without expanding metadata, `Space` toggles useful context, `i` toggles the full audit record, `Enter` opens the source, and `r` reverses chronological order; `#timeline <topic>` and `#timeline open <topic>` prefilter it. `#timeline add [<topic>]` opens a keyboard-first capture sheet without asking the user to classify the event (`Tab`/`Shift+Tab`, `Cmd/Ctrl+Enter` save, `Esc` cancel). Records are one append-only Markdown file each under `.krypton/timeline/events/`, which is gitignored; the sheet requires an explicit **made by** value and keeps it separate from the local recorder/lane. `#timeline trace <topic>` gives the active lane one read-only evidence-tracing turn.
- Local MCP-capable lanes may call `timeline_suggest` once in a turn for an explicit durable requirement, decision, consequential change, approval/rejection, or implementation outcome. This writes only `.krypton/timeline/pending/<reserved-id>.md`; it never records history directly and never starts a sidecar turn. Agent-composed topic, summary, rationale, and impact use natural Thai while evidence, identities, source references, and technical tokens stay verbatim. The active lane head shows a static `timeline N` button while candidates exist. That button or `#timeline review` opens the oldest candidate with its exact evidence and suggesting lane; `Cmd/Ctrl+Enter` confirms the edited event, `Cmd/Ctrl+D` dismisses it, and `Esc` leaves it pending. `#timeline auto`, `auto on`, and `auto off` inspect or change the default-on per-project setting. Existing pending state is retained when off. Remote Harnesses and Pi do not receive automatic proposals; manual add and read-only trace remain the fallbacks. See `docs/254-automatic-timeline-suggestions.md`.
- When the current human message explicitly says to record, remember, persist, or add something to the timeline, a local MCP-capable lane calls `timeline_record` directly. That sentence is the confirmation: no capture/review/permission modal opens. The first event may omit `topic_id`; the result returns the resolved ID, and every remaining event in the same traced chronology reuses it while keeping event-specific wording in `summary`. The final event stores the exact `instruction_excerpt`, attributes the recorder lane, deduplicates retries by resolved topic identity, and promotes a matching pending suggestion under its existing ID. Agent-composed topic, summary, rationale, and impact use natural Thai; the authorizing instruction and provenance stay exact. The lane reports the returned event ID/topic ID/path. This path is independent of `#timeline auto`; a lane must still use `timeline_suggest` when persistence was not explicitly requested. See `docs/255-natural-language-timeline-recording.md` and `docs/261-timeline-reading-and-topic-identity.md`.
- Before that first record, the lane calls the read-only `timeline_list`: with no arguments it returns the stored topics (ID, newest title, event count, time span, newest summary) most-recent first; with `topic_id` it returns that topic's events newest-first. `query` and `limit` bound the result and the response reports the full count plus whether it was truncated. Titles are not identity — the same subject is routinely titled differently per session and per language — so the lane reuses the matching topic's ID instead of creating another topic. It is read-only, auto-approved like the other timeline tools, and available only for a local project-backed Harness. See `docs/262-agent-timeline-topic-discovery.md`.
- `#timeline conflicts` opens a keyboard-first sheet for decisions that may contradict each other (spec 266). `j`/`k` moves between pairs (still-to-review first), and the two events sit side by side with who decided, when, source, and reason. `1` confirmed conflict, `2` not a conflict, `3` insufficient evidence, `4` resolved open a review form with a required rationale; `resolved` also needs the event that ended it or a source reference; `Cmd/Ctrl+Enter` saves and `Esc` backs out. Reviews are appended, never overwritten, and are always attributed to the local user. `n` picks two events from any topic to propose a pair with a reason; `s` starts a TypeSafe screening scan when `[typesafe.timeline_conflicts] mode = "suggest"`. The status line never reads "no conflicts" after a partial or missing scan. In the browser Timeline, rows in a pair still to review (or confirmed and open) get a coloured chip on their time cell only, `made_by` is always shown as recorded, the detail links to the other side and to superseding/superseded events, `p` jumps to the paired event, and `?topic=<id>&event=<id>` deep-links one event. Superseded rows are hidden by default; the button or `h` shows them, and a deep link reveals its target. The page stays read-only. See `docs/266-timeline-decision-trace-and-conflicts.md`.
- `#timeline merge <from> into <into>` moves every event of one topic into another, and `#timeline merge undo` reverses the most recent merge. Each side is an exact `topic-…` ID or a unique case-insensitive substring of a topic title; the separator is the last ` into `. Only `topic_id` changes — titles, summaries, provenance, and event IDs are preserved, and the merged topic keeps showing its newest title. Because `.krypton/` is gitignored, every touched file is copied to `.krypton/timeline/backups/<merge-id>/` with an undo manifest before the first rewrite, and the reported result names the backup and the undo command. Ambiguous, unknown, or identical selectors are refused before anything is written. Unavailable in a remote Harness. The same repair is reachable in plain language: when the human explicitly asks to merge, combine, or join timeline topics — or to undo that — a local MCP-capable lane calls `timeline_merge` with the two `topic_id` values it read from `timeline_list` and the human's exact authorizing words, which are stored in the undo manifest beside the acting lane. That tool is the one timeline tool that is not auto-approved, so the lane asks once before rewriting confirmed records, and it must never merge on its own judgement of similarity. See `docs/263-timeline-topic-merge.md`.
- With `[typesafe].enabled = true` and `[typesafe.timeline_topics].mode = "suggest"`, local add/review sheets may show one semantic existing-topic suggestion after exact matching fails. The request contains only the draft topic/summary and a bounded candidate shortlist; evidence, authorship, lane data, and the API key stay out of frontend/network state. The row is keyboard reachable in normal `Tab` order and requires **ใช้หัวข้อนี้** or **สร้างหัวข้อใหม่** before `Cmd/Ctrl+Enter` can save. `shadow` makes the bounded request without showing the row. Any missing key, weak/ambiguous answer, cancellation, timeout, invalid response, or service error silently preserves the ordinary offline capture flow. See `docs/257-typesafe-timeline-topic-suggestions.md`.
...
| `c` | Transcript focus, or composer with an armed selection | Open the annotate overlay on the selection (or the nearest visible block). |
| `Enter` | Annotate overlay | Add the note to the unsent batch (does not dispatch). |
| `s` | Transcript focus | Send every unsent annotation to the active lane as one turn. |
| `d` | Transcript focus | Delete the last unsent annotation. |
...
| `Cmd+Shift+.` | Any composer/transcript context | Toggle Concise Mode (spec 157). Tool cards collapse to their single head line (glyph + kind + subject + result + live timer; output sections and diff previews hidden); `thought`/`fs_activity`/`memory` rows hide entirely. Interactive rows (`permission`, `fs_write_review`) and everything else stay visible. CSS-only — rows stay in the DOM, streaming continues underneath. Persisted per project; no composer readout — the collapsed cards are the cue (spec 221). Note: the Ctrl+H hidden-row indicator counts transcript rows, so in concise mode N can exceed what visibly appears. See `docs/157-harness-concise-mode.md`. |
...
| `Cmd+P → .` | Harness focused | Open the directive picker (spec 124). `j/k`/arrows move, `Enter` spawns a new lane initialized with the selected directive, `Backspace` clears the focused lane directive, `Esc` cancels. Directives are reusable backend/task-scoped system prompts from `acp-harness.toml`; the active directive injects into the leading context packet and shows as a composer `directive …` chip. Agents can manage directives via `directive_list`/`directive_preview`/`directive_remove`/`directive_apply` MCP tools, with persistent or cross-lane changes gated by an `a`/`r` approval card in the lane transcript. See `docs/124-acp-harness-directive-management.md`. |
| `Cmd+P → '` | Harness focused | Open the review quality matrix overlay (spec 146) — a read-only, per-lane history of `#review` rounds (round · subject · reviewers · blockers · warnings). `j/k` switch the selected lane when more than one has history, `Esc` closes. Disabled (no-op) until at least one round is recorded. Observation, not a score: raw counts only, no jump-to-transcript, no grade. See `docs/146-review-quality-matrix.md`. |
...
- **Permission arrives for a non-active tab:** built-in harness memory/peer MCP permissions are auto-allowed in place only when the permission payload identifies Krypton's harness MCP server (including `krypton-harness-memory`, `krypton-harness-bus`, underscored namespace fragments, rendered `Tool: krypton-harness-bus/peer_send` labels, or `/mcp/harness/` endpoint URLs). Other permissions put that tab's composer in permission mode internally; the active tab's composer is unaffected. The dashboard collapsed row and Zen rail entry become pulsing gold-tinted **action required** surfaces. The user must switch to the tab to resolve. There is no cross-tab approve shortcut by design.
- **Multiple permissions on the same lane:** the lane's composer pre-empts and walks them in order; lane transcript shows one card per request and mutates each card with its decision. If `acceptAllForTurn` / `rejectAllForTurn` is set, subsequent requests resolve automatically until the turn returns; each auto-resolution still updates the corresponding card.
