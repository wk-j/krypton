...
- **Activity ticker (spec 156 / 231)**: the live action (`edit` / `read` / `thinking…` / …) is no longer a composer segment. It paints a compact **action HUD** in the lane rail (`[data-slot="action"]`, stacked above the peek card), with a distinct instrument animation per kind. `lane.activity` is still written as a field on the hot path; the 1 s composer tick and the existing render paths remount or patch the HUD. Thinking is omitted when the thought slot is already showing this lane's live thought. A busy peeked lane reuses the same HUD inside the 109 card. The composer busy chip keeps verb / elapsed / queued only. See `docs/156-lane-activity-ticker.md` and `docs/231-lane-peek-action-hud.md`.
