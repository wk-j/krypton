...
- **Activity ticker (spec 156)**: the busy chip also carries a live activity segment, e.g. `8:23 · ⚒ Edit src/acp/types.ts · 2 queued`. The segment reflects the lane's latest signal — in-flight tool call title (truncated to 64 chars), `thinking…` (thought stream), or `writing…` (answer stream) — stored on `lane.activity` by plain field writes in the event handlers (no per-chunk render; the existing 1 s tick paints). Cleared on stop/error alongside the timer. It is the row's only shrinkable segment: CSS ellipsizes it to the width actually available, so the truncation cap is now just a guard against a pathological title. The output-token counter that used to follow it was removed in spec 221 — the lane stats row prints `in N out N` for the active lane. No new timers or DOM nodes. See `docs/156-lane-activity-ticker.md` and `docs/221-harness-status-line-density.md`.
