...
- **Thought rows** render dimmed and clamped to ~4 lines, with a 5-bar effort meter under the `thought` label (log-scaled by reasoning length) signalling how much is hidden below the clamp. **Veiled thinking:** providers that keep reasoning server-side (Claude Code on current Opus models) stream `thought_chunk`s whose text is empty — the model is thinking but no content ever reaches the client. While such a row streams, the body shows an animated placeholder (`thinking` + pulsing dots, opacity-only animation) instead of an empty block, and the effort meter is suppressed; if the row still has zero text when the stream seals (or the turn errors), it is dropped from the transcript entirely.
...
- **Memory chip** (`memory: 3/3`) sits above the composer line. Counts show up to 10 displayed lane summaries over total lane summaries. During an active turn it changes to the elapsed clock `m:ss` with a subtle lane-accent pulse so long silent agent runs still show liveness. The busy form is built from segments rather than one string (spec 221); the lane name and the `Ctrl+C cancel` hint are **not** repeated here — the lane head carries both for the active lane in every layout, including Zen Mode. Nor is a generic `running` verb: the spinner on the input line, the accented chip and the ticking clock are the busy cue. A **named** operation still leads the chip (`reviewing · 0:41 · …` for a custom command such as `#review`), and `running` survives only as a fallback for the frame before the clock and activity exist.
...
- **Activity ticker (spec 156 / 231)**: the live action (`edit` / `read` / `thinking…` / …) is no longer a composer segment. It paints a compact **action HUD** stack in the lane rail (`[data-slot="action"]`, stacked above the peek card), with a distinct instrument animation per kind — one card per busy lane in the same harness tab. `lane.activity` is still written as a field on the hot path; the 1 s composer tick, background lane renders, and the existing render paths remount or patch the HUD. Thinking is omitted when the thought slot is already showing that lane's live thought. A busy peeked lane reuses the same HUD inside the 109 card and is then skipped in the stack. The composer busy chip keeps verb / elapsed / queued only. See `docs/156-lane-activity-ticker.md` and `docs/231-lane-peek-action-hud.md`.
