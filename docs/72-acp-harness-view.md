...
- The composer is a flush command line (`border-radius: 0`), not an 8px container card. While the harness tab is showing, the host window chrome (frame, titlebar, footer) is also square so the 1px prompt frame is not a rounded capsule. The add-lane picker is a floating overlay: one `.acp-harness__picker-panel` with the theme 8px bevel (not two independently rounded boxes). Other tabs in the same window keep the theme bevel.
...
- **Activity ticker (spec 156 / 231)**: the live action (`edit` / `read` / `thinking…` / …) is no longer a composer segment. It paints a compact **action HUD** stack in the lane rail (`[data-slot="action"]`, stacked above the peek card), with a distinct instrument animation per kind — one card per busy lane in the same harness tab. `lane.activity` is still written as a field on the hot path; the 1 s composer tick, background lane renders, and the existing render paths patch the HUD in place (a new execute does not replay the entrance; remount only when labeled chrome appears or disappears). Thinking is omitted when the thought slot is already showing that lane's live thought. Empty frames hold the last HUD for 2s before the slot hides, so writing/thinking gaps do not remount the card. A busy peeked lane reuses the same HUD inside the 109 card (and drops the activity event row that would restate it) and is then skipped in the stack. The composer busy chip keeps verb / elapsed / queued only. See `docs/156-lane-activity-ticker.md` and `docs/231-lane-peek-action-hud.md`.
