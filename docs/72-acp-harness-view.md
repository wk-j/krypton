...
- The composer is a flush command line (`border-radius: 0`), not an 8px container card. While the harness tab is showing, the host window chrome (frame, titlebar, footer) is also square so the 1px prompt frame is not a rounded capsule. Other tabs in the same window keep the theme bevel.
