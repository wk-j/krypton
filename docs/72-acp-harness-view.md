...
- **Activity ticker (spec 156)**: the busy chip also carries a live activity segment and an output-token counter, e.g. `Claude-1 running · 8:23 · ⚒ Edit src/acp/types.ts · 12.4k tok · Ctrl+C cancel`. The segment reflects the lane's latest signal — in-flight tool call title (truncated to 32 chars), `thinking…` (thought stream), or `writing…` (answer stream) — stored on `lane.activity` by plain field writes in the event handlers (no per-chunk render; the existing 1 s tick paints). Token count is `usage.outputTokens` via `formatCount`, omitted when the backend reports no usage. Cleared on stop/error alongside the timer. No new timers, CSS, or DOM nodes. See `docs/156-lane-activity-ticker.md`.
