...
- Built-in harness memory/peer MCP permissions do not show this banner. The harness automatically chooses the first allow option for `memory_set`, `memory_get`, `memory_list`, `peer_send`, and `peer_list`, then updates the original permission card to `auto-allowed` with a visible harness-rule reason.
