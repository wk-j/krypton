...
- **Current code (`hook_server.rs`):** `artifact_new` (line 522) creates the lane dir + `.gitignore`, records a `pending` entry, and returns `{ id, path, tail, state, title }` — **but never touches the file**. `artifact_register` (line 608) calls `validate_artifact_file` requiring a regular file within `ARTIFACT_FILE_BYTES_MAX` (4 MiB). `artifact_cancel` best-effort deletes the issued file. Caps: `ARTIFACT_TITLE_MAX=200`, `ARTIFACT_PER_SESSION_MAX=512`, `ARTIFACT_PENDING_PER_LANE_MAX=4`.
...
- **No left accent borders (house rule, 2026-06-12).** Left-only `border-left` rails are banned in artifact content — a standing user aesthetic rule (the same one that bans them in Krypton chrome). Enforced at three layers: (1) the scaffold placeholder comment and `artifact_new` tool description tell the lane to color-code blocks with a full border, background tint, or heading/icon color instead; (2) the lane-context stub repeats the rule; (3) a **runtime backstop** `<script>` in the scaffold strips any element inside `main[data-artifact-content]` whose computed style is a left-only border (`border-left-width > 0` while `border-right-width == 0`), via inline `border-left: 0 !important`, on load and on DOM growth (childList-subtree `MutationObserver` — writing only inline `style` cannot re-trigger it). Full borders (`.ka-card`, table cells) are untouched because their right border is non-zero. This is a deliberate exception to the "documented, not blocked" trust envelope: it is a cosmetic normalization, not content inspection.
