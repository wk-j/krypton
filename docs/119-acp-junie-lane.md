...
- **Permission model.** Unknown without manual probing. Junie may emit `session/request_permission` for tool calls (like Claude/Codex/Gemini/Droid) or apply edits directly (like Pi). The `--brave` flag exists but is interactive-only per `junie --help`; Krypton does not pass it. Until verified, the lane shows a `⚠ permissions unverified` chip mirroring the Cursor approach — it's a weaker signal than Pi's `⚠ unsandboxed` chip because Krypton passes no force/yolo/brave flag, but it keeps the safety assumption visible to the user.
...
| Krypton Cursor lane (Spec 113) | `cursor-agent acp` as native ACP child. | Closest precedent — also a regular lane with unverified permission/MCP details. |
...
| `src/acp/acp-harness-view.ts` | Add `junie: 'Junie'` to `BACKEND_LABELS`. **Expand the `accents[]` palette in `laneAccent` from 7 to 8 entries** (today `laneAccent(8)` wraps to `accents[0]` = Codex blue, so a `Junie-1` label without a dedicated arm falls back through `-(\d+)$` to `laneAccent(1)` and collides with Codex). Add `/junie/i.test(label)` arm to `laneAccentForLabel` **before** the numeric `-(\d+)$` fallback (slot 8). Add `⚠ permissions unverified` chip in `renderSandboxChip` for `lane.backendId === 'junie'` until permission semantics are probed; mirror Cursor's tooltip wording about *unverified write autonomy* (not "no permission UI"). Confirm Junie is **not** added to the `pi-acp` no-MCP skip or the `claude` native-MCP skip in `mcpServersForLane()` — defaults are correct for a regular lane. |
...
No `--force` / `--yolo` / `--trust` / `--auto` / `--brave` flags are passed by default. (`--brave` exists in `junie --help` but is interactive-only — irrelevant to ACP children.) Until Junie ACP write-permission behavior is manually verified, the lane chrome shows a `⚠ permissions unverified` chip (mirroring Cursor).

**Tooltip wording.** Mirror Cursor's existing tooltip: communicate that the *write autonomy* is unverified, not that Junie lacks a permission UI. Krypton's permission rail may still light up if Junie emits `session/request_permission`; the chip warns about the *unknown*, not a confirmed absence. Concrete copy: "Junie ACP write-permission behavior has not been verified yet. Krypton does not pass force/yolo/brave flags, but use a trusted cwd until verified."
...
- Lane chip shows `⚠ permissions unverified` until probed (Cursor parity); tooltip uses the "unverified write autonomy" wording, not "no permission UI".
...
- Does Junie ACP emit `session/request_permission` before file writes, deny internally, or mutate directly? (Same scratch-dir probe as Cursor — outcome decides whether the `⚠ permissions unverified` chip stays, is dropped, or is upgraded to `⚠ unsandboxed`.)
...
- `docs/113-acp-cursor-lane.md` — closest sibling spec; same "regular lane + unverified permission/MCP" shape.
