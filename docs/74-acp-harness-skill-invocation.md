...
Selected row gets the lane accent border + slight inverse fill. Mirror the cyberpunk aesthetic — 8px container bevel (`var(--krypton-border-radius, 8px)`), single-pixel glows, no `backdrop-filter: blur()`.
