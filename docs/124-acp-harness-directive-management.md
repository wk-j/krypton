...
- Picker detail pane previews the injected directive block before assignment and may show `estimated_tokens` when available. The preview has a sticky `// prompt` header labelled with the selected directive's title and a bottom fade hinting scrollable overflow. Its height is **fixed** (not content-driven) so navigating between directives of different prompt lengths never resizes or re-centers the overlay — short prompts leave trailing space, long ones scroll.
- The legacy `◆ triage` badge (spec 129 metadata, now display-only per spec 130) is rendered as a demoted quiet inline tag so it does not compete with the directive title.
