...
- **cwd normalization.** Comparisons that decide `cross_project_review` normalize `cwd` before comparing. **As implemented (v1):** `normalizeCwd` strips trailing slashes only — the frontend can't canonicalize symlinks without a Rust round-trip, and the realistic case this guard protects (distinct repository roots) is already separated by string comparison. Consequence: two views opened on the *same* repo through *different symlinked paths* would currently read as cross-project and a review between them would be refused. Full symlink canonicalization (a Rust `canonicalize` command) is a noted follow-up if that case arises; it does not affect plain `peer_send`, which has no cwd guard.
