...
3b. **Handles** `_x.ai/ask_user_question` / `x.ai/ask_user_question` by parking a oneshot and showing an inline question card (spec 229). Do **not** auto-answer. Reply `{ type: "accepted", answers: [{ question, selected_labels }], partial_answers: null }` or `{ type: "skip_interview" }`. First-attempt wire; live-verify against grok 1.0.4 before calling the shape final.
