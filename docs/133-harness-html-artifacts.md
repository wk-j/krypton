...
Use a **path-handoff** protocol (the lane writes the file; the harness never transports HTML bytes). A lane calls `artifact_new { title }` on the existing lane-scoped `krypton-harness-bus` server; the harness allocates an artifact id and returns a destination path **inside the target project** at `.krypton/artifacts/<harnessId>/<laneId>/<artifactId>.html`. The lane writes the HTML to that path with its **normal file-write tool**, then calls `artifact_register { id }`. The harness validates the file (see Security), records the artifact attributed to the lane, and renders a hintable artifact card in the transcript. The user opens it with a hint label (same label system as the existing `f` hint mode), and the harness opens the artifact in the **OS browser**.

> **Open mechanism amended by spec 149.** The harness no longer opens the artifact as a `file://` path. It opens a **served loopback URL** — `http://127.0.0.1:<harnessMemoryPort>/artifact/<token>`, where the harness-memory HTTP server serves the registered artifact's HTML from disk on each request (re-running the full validation policy). The OS-browser surface, the hint-keypress gate, and never-auto-open are all unchanged (see ADR 0002, amended). Each `artifact_new` also bakes an unguessable per-artifact **feedback token** + the loopback base URL into the seeded scaffold and records `token → {harnessId, laneLabel, artifactId}` in the registry. This gives the page a real HTTP origin so the spec-149 inline-feedback overlay can `fetch` same-origin (no CORS). Tokens are revoked forward-only on lane close / `#new` (a later request gets `410`); `#restart` keeps the token so feedback routes to the respawned session. See `docs/149-artifact-inline-feedback.md`.
...
|------|---------|----|-------|
| (none) | `artifact_new` (caller lane) | `pending` | Fails if the lane's pending cap is hit. Opens write auto-approval for the issued path. **The issued path is pre-populated with a styled scaffold ([spec 134](134-artifact-default-styling.md)) — not empty; the lane *edits* it, and registering the untouched scaffold is allowed.** |
