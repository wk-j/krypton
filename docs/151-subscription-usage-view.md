...
- **Cursor** (added post-implementation, user request) — `GET https://cursor.com/api/usage?user=<id>` with cookie `WorkosCursorSessionToken=<userId>%3A%3A<jwt>`; user id from `~/.cursor/cli-config.json` `authInfo`, JWT from macOS Keychain service `cursor-access-token`. **The CLI token only unlocks legacy request counters** (null/zero on current usage-based plans; the dashboard quota APIs return 403 for non-browser sessions — verified), so the widget usually reports "connected · quota not exposed" and only shows a request gauge on legacy capped plans.
...
- `usage_fetch_cursor() -> Result<CursorUsage, String>` — user id from `cli-config.json` + Keychain JWT (macOS only; elsewhere `not-connected`), GET `cursor.com/api/usage`, 180 s cache, same ladder.
