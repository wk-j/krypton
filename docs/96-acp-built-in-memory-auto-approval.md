...
1. The permission request contains one of the allowed built-in tool names (memory: `memory_set`, `memory_get`, `memory_list`; peer: `peer_send`, `peer_list`; attention: `attention_flag`, `attention_resolve`; review: `review_outcome`) somewhere in `toolCall.{rawInput,title,content}` or in any `permission.options[].name`. (The spec-112 `review_request`/`review_reply` tools were removed in spec 145; `#review` now rides `peer_send`. The narrower `review_outcome` self-report tool was added in [spec 146](146-review-quality-matrix.md) — default-on, so a permission prompt mid-synthesis is a bug.) (Attention triage is default-on built-in harness tooling per [spec 130](130-default-attention-triage.md); a permission prompt for it is a bug *and* breaks the non-blocking contract — the lane has already proceeded with `chosen`.)
