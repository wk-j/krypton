...
- **ACP agent windows (Leader A)** — second, parallel agent window backed by the [Agent Client Protocol](https://agentclientprotocol.com). Spawns an external adapter (Claude Code, Gemini CLI, …) over newline-delimited JSON-RPC and surfaces tool calls, plans, thoughts, and inline permission prompts. Lives in `src/acp/` + `src-tauri/src/acp.rs`; the existing pi-agent at `src/agent/` is untouched. Configure via `[acp.<id>]` blocks in `krypton.toml`. See `docs/69-acp-agent-support.md`.
