...
> Status: Implemented (rev 21 — the rail thought card preserves its live teletype DOM at seal; rev 20 — worker-lane idle ticket reload patches the dock instead of remounting the dashboard; rev 19 — thought seal preserves the live text node and native wrapping; rev 18 — transcript entrance transforms release their WebKit compositing backing; rev 17 — the first assistant/user chunk seals a live thought before clearing its stream pointer; rev 16 — sealed table/reference layout changes cannot unstick a pinned transcript; rev 15 — smooth-follow does not replay a glide when the scroll target is unchanged; rev 14 — composer key edits pin a stuck transcript in the same turn; rev 13 — a scroll-sourced sticky-drift heal pins synchronously, before paint; rev 12 — sticky-drift heal: a stuck lane short of the bottom is re-pinned; rev 11 — keyboard scroll up always unsticks; rev 10 — scroll stickiness measured at event dispatch, committed in the RAF; rev 9 — usage and plan events stay off the full lane rebuild; rev 8 — streaming tool output patches the output block in place; rev 7 — streaming smooth-follow scroll; rev 6 — stop and MCP stats stay off the full dashboard rebuild; thought/user seal matches assistant)
> Date: 2026-05-22
...
>
> Amended (rev 17) by: `appendStreaming` captures and seals the live thought row before an assistant/user chunk clears `currentThoughtId`. ACP has no separate thought-finished event, so that next stream kind is the boundary; stamping the sealed signature there prevents the following body-only pass from rebuilding the thought row with `replaceChildren`.
>
> Amended (rev 18) by: transcript row entrance animations no longer forward-fill their `transform`, and each row removes `acp-harness__msg--enter` on `animationend` or `animationcancel`. A live Retina macOS trace was consistent with finished rows retaining WebKit graphics backing while the 60-row tail window grew; clearing the effect removes the row-level compositing trigger so WebKit can demote and release those layers. Reduced-motion disables the entrance animation entirely.
>
> Amended (rev 19) by: sealing a live thought keeps its existing plain-text node and native wrapping instead of marking the body for a second Pretext layout. Rev 17 preserved the wrapper at the thought→assistant boundary, but `sealStreamingTextBody` still replaced the identical text node and the following layout pass rebuilt its children. The thought→assistant, thought→tool, and thought→stop paths all share this seal helper, so preserving the live DOM removes that completion-boundary repaint without changing user/assistant sealing.
>
> Amended (rev 20) by: `reloadActiveTicket` / `enrichActiveTicket` patch the Ticket Panel (`renderTicketDock`, and the `#ticket` overlay when it is open) instead of `this.render()`. Rev 6 already kept `stop` off the full lane rebuild, but `setLaneStatus(idle)` still reloaded the working ticket for the bound worker lane and remounted `dashboardEl` — a whole-harness flash a beat after turn end. Missing-ticket still full-renders so layout classes drop. Live `acp-ticket-progress` was already dock-only.
>
> Amended (rev 21) by: the separate rail thought card settles its live teletype spans in place when `delta` becomes `seal`. It fills the existing ghost span with the final collapsed text, empties the fresh span, removes the caret, and records the sealed-plain source so later idle/chrome refreshes remain no-ops. A sealed thought mounted later from history still renders as GFM. This removes the last known turn-completion DOM replacement without changing historical presentation.
...
> Amended (rev 16) by: sealed-assistant layout suppression (§2/§5). Finalising a live assistant row may replace streaming markup with a GFM table and append the deterministic reference rail. Those synchronous DOM mutations change the transcript height before the body-only follow pass; a browser-generated scroll event in that gap could be classified as user intent and set `stickToBottom = false`, so the pending follow pass became a no-op. When the active lane is already sticky, `sealStreaming` now holds the existing programmatic-scroll suppression across `sealAssistantStreamingMarkdown`; the normal two-RAF release and upward-wheel escape remain unchanged. A reader who deliberately scrolled up is not suppressed because the guard is only entered for a sticky lane.
