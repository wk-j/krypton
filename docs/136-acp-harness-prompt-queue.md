...
- **`renderActiveLaneQueue()`** — new method that reads the **active** lane's `queuedPrompts`; if empty, `queueSlotEl.hidden = true` and return; otherwise renders a `.acp-harness__lane-queue` block:
...
- **Zen mode (Grok-1 finding; revised post-v1).** `laneRailEl` is re-parented into the active lane card on every `renderLanes` regardless of zen, and there is no zen rule repositioning `.acp-harness__lane-rail`. The active lane shell (head + stats + transcript body + `laneRailEl`) renders inside the zen `.acp-harness__body-cell`, so the bottom-rail queue slot is already present in the DOM there. `renderActiveLaneQueue()` therefore does **not** gate on `this.zenMode` — the active lane's queue follows it into zen and shows in the bottom rail, same chrome as normal mode. (The zen left rail / `refreshZenRail` shows the per-lane summary; queue depth there is out of scope.)
...
- **Zen mode:** `renderActiveLaneQueue()` no longer gates on `zenMode` — the bottom-rail queue slot lives inside the active lane shell, which renders in the zen body-cell, so the active lane's queue shows in zen with the same chrome as normal mode. (v1 hid it; revised.) ✔
