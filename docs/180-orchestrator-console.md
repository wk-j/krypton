...
### Collapse around other modals

The console is a full-surface overlay, but other harness modals reach the user through the **leader menu** (`Cmd+P → key`, handled by the InputRouter) which bypasses the console's own key capture — so a lane/session/directive/model picker, the triage / review-matrix / review-priority overlays, or the metrics panel can open *on top of* an already-open console. To avoid two full-screen surfaces stacking, the console **collapses** (hides) while any such modal is open and **restores** (repainted from current state) when it closes. Only visibility changes — `orchestratorConsoleOpen` (the seat, the `j/k` card selection, the `a/r` permission target) is untouched, so dismissing the modal lands the operator back exactly where they were.

Mechanics: `consoleObscuringModalOpen()` ORs the modal-open flags; `syncOrchestratorConsoleVisibility()` hides-or-restores and is called from `render()` (covers the pickers, help, and memory drawer, which all re-render through it) and from the four overlays that render outside `render()` (triage / review-matrix / review-priority open+close, metrics toggle). `renderOrchestratorConsoleEl()` also folds the predicate into its `hidden` computation so a background `LaneBus` refresh never un-hides a collapsed console. The console's key capture is gated on `!consoleObscuringModalOpen()` so that a modal checked *below* it in the `onKeyDown` chain (metrics `Esc`, memory drawer) receives keys while it is the visible top surface, rather than the hidden console swallowing them.
