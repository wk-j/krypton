...
| FR-INP-037 | The system shall support cycling workspace layout modes with `Leader+F`: **Grid** → **Focus** → **Depth** → **Scroll** → **Stage** → Grid. Focus layout puts the focused window in the left column at full height and keeps both columns inset from the screen edge by the configured window gap. Scroll layout (spec 241) places windows on a horizontal strip without resizing existing columns. Stage layout (spec 245) keeps one active window large with recent live previews on a left shelf. | Should |
...
| FR-WIN-010 | The system shall support **pinning** a window via keybinding (`Leader p` toggle). A pinned window sticks to the right column in Focus layout and remains reachable during clockwise/counterclockwise focus cycling (`Cmd+Shift+</>`) without moving to the main column while an unpinned window exists. | Should |
...
| FR-WIN-013 | In Grid and Focus layouts, `Cmd+Shift+>` shall cycle focus clockwise through every shown window and `Cmd+Shift+<` shall cycle counterclockwise. Grid order shall derive from current on-screen geometry rather than window creation order. | Should |
...
| FR-ACP-006 | The ACP Harness shall expose lane-scoped MCP memory tools, render lane memory as a read-only human board, and prepend a short lane-context stub (identity + roster + MCP-tools nudge) to future prompts so agents pull memory bodies on demand. | Should |
...
| FR-ACP-008 | The ACP Harness shall stage pasted, dropped, and global screen-captured images into the active lane composer, render removable image placeholders before submit, and send them as embedded ACP image content blocks with base64 data and a local file URI with the next prompt. | Should |
| FR-ACP-009 | When a contextual lane peek is visible, the ACP Harness shall expose command-palette actions to cycle peek heat metric, cycle peek heat time window, and toggle expanded heat detail; the heat strip shall remain keyboard-addressable (no mouse-only controls). | Should |
| FR-ACP-010 | A visible ACP or ACP Harness content view shall show supported providers' subscription quota percentages in its host window's bottom status bar; multi-lane harness providers shall be deduplicated, and constrained widths shall collapse each provider to its most-used quota window. | Should |
| FR-ACP-011 | The system shall expose authenticated local CLI control for capability-advertised ACP Harness domain operations without impersonating a lane or simulating UI key/hash commands. | Should |
| FR-ACP-012 | On macOS, `Ctrl+Shift+I` shall summon or hide a separate Live Assist native window centered in the pointer display's usable work area, above normal application windows on the active Space. | Must |
| FR-ACP-013 | Summoning, using, and hiding Live Assist shall not resize, move, focus or raise, reparent content from, or change workspace/tab/pane state in the main Krypton window. Live Assist itself takes focus while shown. | Must |
| FR-ACP-014 | Live Assist shall use its own renderer and local composer draft while the existing `AcpHarnessView` remains the sole owner of ACP clients, lane state, transcripts, and permissions. | Must |
| FR-ACP-015 | Live Assist shall support lane selection, transcript tail viewing, prompt send/queue, turn cancellation, and oldest-request permission accept/reject from the keyboard. | Must |
| FR-ACP-016 | Live Assist shall reuse the typed internal Harness control dispatcher and ordered event stream without loopback HTTP credentials or a second lane observer. | Must |
| FR-ACP-017 | When no Harness is open, Live Assist shall show an informative empty state and shall not create or focus a Harness automatically. | Must |
| FR-ACP-018 | Each changed file in an assistant response's `REFERENCES` rail shall show current Git status and per-file added/removed line counts derived programmatically from the local working tree, never from assistant/model-provided count metadata. | Must |
| FR-ACP-019 | Each window's status bar shall show one backend mark per lane of the ACP Harness in that window's focused pane, highlighting the active lane and identifying it by a rendered lane name in addition to colour, so every window reports its own lanes independently. | Should |
| FR-ACP-020 | Each window's status bar shall name the project directory of that window's focused content view, with the name's leading characters magnified so several tiled windows can be told apart at scanning distance; the untruncated path shall remain available as the element's tooltip and accessible label. | Should |
| FR-ACP-021 | Beside that name, each window's status bar shall report the uncommitted line volume of the focused view's Git repository as added and removed counts against `HEAD` — staged, unstaged and untracked alike — magnified to match its neighbours. The counts shall keep themselves current without user action, updating both on a bounded interval and at the moment a harness lane in that repository finishes a turn, and shall be absent when the focused pane has no repository or its tree is clean. | Should |
| FR-ACP-022 | The system shall compose, on demand, a dated developer note for a single day from records it already keeps — per-turn usage, Git history, review bundles, generated artifacts, and a journal of harness events — reachable by keybinding, command palette, and harness command. Every statement in the note shall be traceable to one of those records: the note shall contain nothing written, summarised, or inferred by a language model, and shall name its own sources. Narration of the note shall be a separate, explicitly requested action whose output is never written into it. A note the user has edited by hand shall never be overwritten by a later regeneration. | Should |
| FR-ACP-023 | The ACP Harness shall offer opt-in speech-to-text dictation for the active text composer through `Cmd+D` (English), `Cmd+Shift+D` (Thai), and a visible microphone control (English) when the host webview supports speech recognition. Dictation shall preview interim text, require a separate `Enter` to submit, and restore the saved draft when cancelled or torn down. | Should |
| FR-ACP-024 | The ACP Harness shall show prompt-cache hit rate from one coherent turn tuple in the active lane stats and from recorded day totals in `#usage` and `#daily`; when required counters are absent, it shall show raw counts or no rate rather than infer one from merged or missing data. A successful non-empty `#usage` read shall render as a structured telemetry card with visible raw counters, token composition, and per-model rows while retaining an equivalent plain-text fallback for projections that do not render the card. | Should |
| FR-ACP-025 | The ACP Harness composer shall acknowledge direct text insertion with a configurable fading letter afterimage: immediate draft and caret, overlay copies of the inserted graphemes (no expanding ellipse), duration and trail length in `krypton.toml`, and no effect under system Reduce Motion or `composer_bloom = false`. | Should |
| FR-ACP-026 | The ACP Harness shall provide a built-in `#timeline` command that records append-only project-change events without requiring the user to classify them, stores them under the gitignored project-local `.krypton/timeline/events/` directory, browses them through a read-only keyboard-operated loopback page whose chronology stays compact until context or audit detail is explicitly opened, and traces older history read-only without treating code authorship as decision authority. | Should |
| FR-ACP-027 | For a local MCP-capable Harness lane, the system shall let the working agent propose at most one important timeline candidate per turn into gitignored pending storage, show the project-wide pending count without stealing focus, and require a human to edit/confirm or dismiss the candidate before it becomes an append-only timeline event. Automatic proposals shall be enabled by default, controllable per project with `#timeline auto [on|off]`, and preserve the exact supporting evidence plus suggesting lane. | Should |
| FR-ACP-028 | When the current human message explicitly asks a local MCP-capable Harness lane to record, remember, persist, or add an event to the project timeline, the lane shall be able to append the authoritative event in the same turn without capture, review, or permission UI. The event shall preserve the exact authorizing instruction and recorder lane, report its event ID/topic ID/path in the conversation, reuse an explicit existing topic ID across a traced chronology, deduplicate retries by resolved topic identity, and promote a matching pending suggestion in place; unsolicited agent-selected events shall remain proposal-first. The lane shall also be able to read the existing topics and one topic's events read-only before recording, and shall be told when a record created a new topic together with the topics that already existed, so one subject is not split across topics by title or language differences between sessions. The human shall additionally be able to merge one topic's events into another from the keyboard and undo that merge, with the affected records backed up automatically before any rewrite; a lane shall perform the same merge only on an explicit human instruction whose exact words are stored with the backup, and that path shall not be auto-approved. | Should |
| FR-ACP-029 | When explicitly enabled for a local Harness, Timeline add and suggestion-review sheets shall use a bounded, server-side TypeSafe `Choice` request to offer one semantic existing-topic match only after deterministic exact matching fails. The user shall explicitly choose that topic or create a new one before save; missing credentials, insufficient coverage or confidence, cancellation, timeout, invalid responses, and service failure shall preserve the offline capture path without exposing credentials or provenance. | Should |
| FR-ACP-030 | When TypeSafe is enabled and has started at least one outbound request, the Workspace Footer shall show the exact process-lifetime count of HTTP attempts, including retries, and the existing Profiler shall expose operations, retries, settled outcomes, and average latency from the same aggregate in-memory snapshot without polling or exposing request content. | Should |
| FR-ACP-031 | The project timeline shall show `supersedes` history as a chain without editing past events, and shall let the human propose two events that may conflict, compare them side by side, and record an append-only verdict (confirmed, not a conflict, insufficient evidence, resolved) with a required rationale from the keyboard. Rows in a pair still to review or confirmed and open shall be marked in the read-only browser page without claiming which side is right. An optional, user-started TypeSafe scan may propose pairs but shall never record a verdict, and a partial or missing scan shall never be reported as zero conflicts. | Should |
