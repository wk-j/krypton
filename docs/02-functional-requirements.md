...
| FR-ACP-019 | Each window's status bar shall show one two-letter mark per lane of the ACP Harness in that window's focused pane. The active mark shall be magnified and show a small trailing lane number when present; its full name shall remain available in the tooltip and accessible label. Every window reports its own lanes independently. | Should |
...
| FR-ACP-031 | The project timeline shall show `supersedes` history as a chain without editing past events, and the lane agent shall find conflicting decisions, record an append-only verdict (confirmed, not a conflict, insufficient evidence, resolved) with a rationale, and close pairs itself — the human never picks pairs or fills verdict forms (spec 267). Checks shall be incremental: only events no agent has compared yet are read. Rows in a pair still to review or confirmed and open shall be marked in the read-only browser page without claiming which side is right. | Should |
