...
| FR-ACP-028 | When the current human message explicitly asks a local MCP-capable Harness lane to record, remember, persist, or add an event to the project timeline, the lane shall be able to append the authoritative event in the same turn without capture, review, or permission UI. The event shall preserve the exact authorizing instruction and recorder lane, report its ID/path in the conversation, deduplicate retries, and promote a matching pending suggestion in place; unsolicited agent-selected events shall remain proposal-first. | Should |
